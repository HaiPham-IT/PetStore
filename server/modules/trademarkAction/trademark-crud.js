...
const createTrademark = (req, res, next) => {
    ...
    return Trademark.findOne(trademark).exec()
            .then(rs => rs ?  createErr(`Trademark [${trademark.name}] already exist`, 409) : new Trademark(trademark).save())
...
const deleteTrademark = (req, res, next) => {
    return Trademark.deleteOne({name: req.params.name})
            .then(rs => rs.deletedCount > 0 ? res.status(200).json(rs) : createErr(`Trademark [${req.params.name}] not found`, 404))
            .catch(err => next(err))
}
